use crate::layout::group_box_label_height;
use crate::{
...
fn clip_triangle(points: &mut [[f32; 2]; 3], clip: UiRect) -> bool {
    // The renderer accepts triangles only.  Keep the first clipped triangle;
    // preview artwork uses small independent triangles, so this avoids visual
    // leakage at a scroll edge without introducing a new tessellation format.
