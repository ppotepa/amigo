...
#[test]
fn npr_stroke_strip_smoothed_direction_handles_kinked_samples() {
    let viewport = Viewport::from_dimensions(800.0, 600.0);
    let samples = [
        NprStrokeStripSample {
            point: Vec2::new(-0.30, -0.05),
            width_px: 4.0,
            offset_px: 0.0,
            overshoot_px: 0.0,
            color: ColorRgba::WHITE,
    ...
    let direction = npr_stroke_smoothed_direction(
        &samples,
        &viewport,
        1,
        amigo_render_api::NprTessellationProfile3d::smoothed_strip(),
    )
    .expect("kinked sample should still produce a stroke direction");
