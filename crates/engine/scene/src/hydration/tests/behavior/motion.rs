...
"#####,
    ...
    assert!(plan.commands.iter().any(|command| {
        super::plugin_payload::<crate::FreeflightMotion2dSceneCommand>(command).is_some_and(
            |command| {
                command.entity_name == "curve-ship"
...
"#####,
    ...
    assert!(plan.commands.iter().any(|command| {
        super::plugin_payload::<crate::InputActionMapSceneCommand>(command).is_some_and(|command| {
            command.entity_name == "gameplay-controls"
                && command.id == "gameplay"
                && command.active
...
"#####,
    ...
    assert!(plan.commands.iter().any(|command| {
        super::plugin_payload::<crate::BehaviorSceneCommand>(command).is_some_and(|command| {
            command.entity_name == "ship-controller"
