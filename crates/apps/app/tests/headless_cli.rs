...
fn app_headless_cli_runs_playground_npr_comic_lines_scene() {
    ...
    assert!(stdout.contains(
        "scene document components: Camera3D x1, CameraController3D x1, InputActionMap x1"
    ));
    ...
    assert!(stdout.contains(
        "3d.mesh.apply_npr_preset(playground-npr-model-1-soldier, toriyama_ink_cpu_reference)"
    ));
    assert!(stdout.contains("Backend  cpu_reference | CPU only"));
