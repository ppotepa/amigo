...
pub(crate) enum DebugOverlayPanel {
    Fps,
    FpsGraph,
    Stats,
    Particles,
...
pub(crate) enum DebugOverlayLayoutMode {
    Compact,
    Full,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum DebugOverlayCorner {
    TopLeft,
    TopRight,
    BottomLeft,
    BottomRight,
...
#[derive(Debug, Clone)]
pub(crate) struct DebugOverlaySettings {
    pub(crate) enabled: bool,
    pub(crate) layout_mode: DebugOverlayLayoutMode,
    pub(crate) corner: DebugOverlayCorner,
    pub(crate) scale: f32,
    pub(crate) panels: BTreeSet<DebugOverlayPanel>,
}
...
pub(crate) struct DebugOverlayService {
    state: Mutex<DebugOverlayState>,
}

impl DebugOverlayService {
    pub(crate) fn snapshot(&self) -> DebugOverlaySnapshot {
        let state = self
            .state
            .lock()
    ...
    pub(crate) fn record_render_frame(&self, stats: RenderFrameStats) {
        let now = Instant::now();
        let mut state = self
            .state
            .lock()
    ...
    pub(crate) fn record_scheduling_stats(&self, stats: SchedulingFrameStats) {
        self.state
            .lock()
            .expect("debug overlay mutex should not be poisoned")
    ...
    pub(crate) fn record_audio_snapshot(
        &self,
    ...
    pub(crate) fn record_input_snapshot(
        &self,
    ...
    pub(crate) fn record_particle_snapshot(&self, emitter_count: usize, active_emitters: usize) {
        self.state
            .lock()
            .expect("debug overlay mutex should not be poisoned")
            .latest_particle_snapshot = DebugOverlayParticleSnapshot {
    ...
    pub(crate) fn set_panel_visible(&self, panel: DebugOverlayPanel, visible: bool) {
    ...
    pub(crate) fn toggle_panel(&self, panel: DebugOverlayPanel) -> bool {
        let mut state = self
            .state
            .lock()
            .expect("debug overlay mutex should not be poisoned");
    ...
    pub(crate) fn set_layout_mode(&self, mode: DebugOverlayLayoutMode) {
    ...
    pub(crate) fn set_corner(&self, corner: DebugOverlayCorner) {
