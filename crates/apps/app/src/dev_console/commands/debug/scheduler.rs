...
impl ConsoleCommandHandler for DebugSchedulerCommandHandler {
    fn name(&self) -> &'static str { "debug-scheduler" }
    ...
    fn descriptors(&self) -> Vec<ConsoleCommandDescriptor> {
        vec![ConsoleCommandDescriptor {
            ...
            examples: &["debug.scheduler on", "debug.scheduler off", "debug.scheduler"],
            dev_only: true,
        }]
    }

    fn can_handle(&self, command: &ParsedConsoleCommand) -> bool { command.name == "debug.scheduler" }

    fn handle(&self, ctx: &ConsoleCommandContext<'_>, command: ParsedConsoleCommand) -> ConsoleCommandResult {
        apply_panel_toggle(ctx, &command, DebugOverlayPanel::Scheduler, "debug.scheduler")
    }
}
