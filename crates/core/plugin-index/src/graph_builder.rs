...
fn add_participation_nodes(
    graph: &mut CodeMapGraph,
    manifest: &PluginManifest,
    plugin_node: &CodeMapNodeId,
) {
    let owns_candidate = matches!(
        manifest.kind,
        PluginKind::RenderableSource | PluginKind::SemanticSource | PluginKind::SourceAndConsumer
    );
    if owns_candidate {
        let candidate = CodeMapNodeId::Candidate(format!("{}::candidate", manifest.id.0));
        graph.add_node(CodeMapNode::new(
            candidate.clone(),
            CodeMapNodeKind::Candidate,
            format!("{} candidate", manifest.id.0),
        ));
        graph.add_edge(CodeMapEdge::new(
            plugin_node.clone(),
            candidate,
            CodeMapEdgeKind::ResolvesCandidate,
        ));
    }

    let owns_consumer = matches!(
        manifest.kind,
        PluginKind::TargetConsumer | PluginKind::SourceAndConsumer | PluginKind::Adapter
    );
    if owns_consumer {
        let consumer = CodeMapNodeId::Consumer(format!("{}::consumer", manifest.id.0));
        graph.add_node(CodeMapNode::new(
            consumer.clone(),
            CodeMapNodeKind::Consumer,
            format!("{} consumer", manifest.id.0),
        ));
        graph.add_edge(CodeMapEdge::new(
            plugin_node.clone(),
            consumer.clone(),
