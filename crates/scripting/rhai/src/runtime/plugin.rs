...
fn build_engine(
    ...
    crate::bindings::controls::register(&mut engine, runtime_control.clone());
        ...
        let numbers: Option<Vec<f32>> = array
            ...
            .map(|v| {
                v.as_float()
                    .ok()
                    .map(|n| n as f32)
                    .or_else(|| v.as_int().ok().map(|n| n as f32))
            })
            .collect();
        match numbers.as_deref() {
            Some([x, y]) => ControlValue::Vec2([*x, *y]),
            Some([x, y, z]) => ControlValue::Vec3([*x, *y, *z]),
            Some([r, g, b, a]) => ControlValue::Color([*r, *g, *b, *a]),
            _ => ControlValue::Null,
        }
    } else {
...
fn control_value_to_dynamic(value: ControlValue) -> rhai::Dynamic {
    match value {
        ...
        ControlValue::Vec2(v) => v
            .into_iter()
            .map(|x| rhai::Dynamic::from_float(x as rhai::FLOAT))
            .collect::<rhai::Array>()
            .into(),
