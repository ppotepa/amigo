use amigo_render_api::{
    CameraCaptureInput2d, CameraOpticalCandidate2d, CameraOpticalCandidateStatus2d,
    CameraOpticalCoverage2d, CameraOpticalResponse2d, LightContributionKind2d,
    LightEmitterKind2d, LightSource2dCommon, LightSourceStatus2d, Material2d,
    RenderContributionSet, VisualSourceAvailability2d,
};
...
pub fn collect_camera_optical_candidates_2d(
    ...
    light_sources
        .iter()
        .filter_map(camera_optical_candidate_from_light_source)
        .collect()
}
...
fn camera_optical_candidate_from_light_source(
    ...
    let source_enabled = source.status == LightSourceStatus2d::Active;
    let role_enabled = source
        .contributions
        .iter()
        .any(|kind| matches!(kind, LightContributionKind2d::CameraFxSource | LightContributionKind2d::BloomSource));
    let response_enabled = response.enabled
        && response
            ...
            .max(response.halation)
            > 0.0;
    let coverage = optical_coverage_for_light_source(source);
    let coverage_supported = coverage.is_supported();
    let active = source_enabled && role_enabled && response_enabled && coverage_supported;
    let reason = if !source_enabled {
        source.reason.clone()
...
fn optical_coverage_for_light_source(source: &LightSource2dCommon) -> CameraOpticalCoverage2d {
    match source.emitter_kind {
        LightEmitterKind2d::LightGroup => light_group_coverage(source),
        LightEmitterKind2d::Beacon => CameraOpticalCoverage2d::Hotspot {
            entity_name: source.owner.clone(),
            radius_px: source.radius_px.unwrap_or(0.0),
        },
        LightEmitterKind2d::ParticleLight => CameraOpticalCoverage2d::ParticleCoverage {
...
fn light_group_coverage(source: &LightSource2dCommon) -> CameraOpticalCoverage2d {
    let Some(emitter_id) = source.emitter_id.as_deref() else {
        return CameraOpticalCoverage2d::Unsupported {
            reason: "missing_light_group_source_id".to_owned(),
        };
    };
    let parts = emitter_id.split(':').collect::<Vec<_>>();
    if parts.len() == 4 && parts[1] == "lightmap" {
        return CameraOpticalCoverage2d::LightMapChannel {
            source: parts[2].to_owned(),
            channel: parts[3].to_owned(),
        };
    }
    CameraOpticalCoverage2d::Unsupported {
        reason: "light_group_source_not_lightmap_channel_v1".to_owned(),
    }
}
...
mod tests {
    ...
    fn camera_optical_candidates_report_light_group_lightmap_coverage() {
        ...
        let candidates = collect_camera_optical_candidates_2d(&light_sources);
        let summary = format_camera_optical_candidates_2d(&candidates);
    ...
    fn camera_optical_candidate_unsupported_coverage_is_skipped() {
        let source = amigo_render_api::LightSource2dCommon {
            ...
            response: Some(1.0),
            camera_response: Some(amigo_render_api::CameraOpticalResponse2d {
                enabled: true,
                intensity: 1.0,
                glare: 1.0,
                bloom: 1.0,
        ...
        let candidates = collect_camera_optical_candidates_2d(&[source]);
        let summary = format_camera_optical_candidates_2d(&candidates);

        assert_eq!(candidates.len(), 1);
        assert_eq!(
            candidates[0].status,
            amigo_render_api::CameraOpticalCandidateStatus2d::Skipped
        );
