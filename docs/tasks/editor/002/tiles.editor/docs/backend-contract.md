...
2. Read YAML.
3. Require a typed descriptor schema.
4. Normalize descriptor YAML into SheetResourceDto.
...
Legacy `*.semantic.yml` is not supported as an active editor schema.

## 7. Saver behavior

MVP saver writes normalized descriptor schema back to the same typed descriptor path:

```txt
save_sheet_resource(session_id, resource_uri, dto)
```

Saving to untyped or legacy paths should return an error.
